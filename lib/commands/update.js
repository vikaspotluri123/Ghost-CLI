    ...
    run(argv) {
        ...
        return instance.running().then((isRunning) => {
            ...
            return this.ui.listr([{
                ...
                task: () => (this.runCommand(DoctorCommand, Object.assign(
                    {quiet: true, categories: ['update']},
                    argv)))
            }, {
                title: 'Running CLI migrations if needed',
                task: () => this.runCommand(MigrateCommand, {quiet: true})
            }, {
                title: 'Checking for latest Ghost version',
                task: (ctx) => this.version(context).then(result => ctx.result = result)
            }]).then((listrCtx) => {
                if (!listrCtx.result) {
