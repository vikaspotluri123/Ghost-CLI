...
class UI {
    ...
    constructor(options) {
        ...
        // Add custom prompt module that uses the specified streams
    ...
    /**
     * Prompts a user for input. Uses enquirer to handle prompts
     *
    ...
    prompt(prompts) {
        if (!this.allowPrompt) {
            throw new SystemError('Prompts have been disabled, all options must be provided via command line flags');
        }
        ...
        if (this.auto) {
            if (Array.isArray(prompts)) {
                ...
                return this.noSpin(() => this.enquirer.prompt(promptsToAsk))
        ...
        return this.noSpin(() => this.enquirer.prompt(prompts));
    }
